...
public class AxonProcessor {
    ...
    void configureRuntimeProperties(AxonRuntimeTemplate template, AxonRuntimeConfig axonRuntimeConfig, BeanContainerBuildItem beanContainer) {
        template.setAxonBuildConfig(beanContainer.getValue(), axonRuntimeConfig);
    }

    @BuildStep
    @Record(ExecutionTime.RUNTIME_INIT)
    void registerSagas(AxonRuntimeTemplate template, List<SagaBuildItem> axonAnnotatedBeans, BeanContainerBuildItem beanContainer) {
    ...
    @BuildStep
    @Record(ExecutionTime.RUNTIME_INIT)
    void registerAggregates(AxonRuntimeTemplate template, List<AggregateBuildItem> axonAnnotatedBeans, BeanContainerBuildItem beanContainer) {
        axonAnnotatedBeans.forEach(item -> template.registerAggregate(beanContainer.getValue(), item.getAxonAnnotatedClass()));
    }

    @BuildStep
    @Record(ExecutionTime.RUNTIME_INIT)
    void registerCommandHandlers(AxonRuntimeTemplate template, List<CommandHandlerBuildItem> axonAnnotatedBeans, BeanContainerBuildItem beanContainer) {
        axonAnnotatedBeans.forEach(item -> template.registerCommandHandler(beanContainer.getValue(), item.getAxonAnnotatedClass()));
    }

    @BuildStep
    @Record(ExecutionTime.RUNTIME_INIT)
    void registerEventHandlers(AxonRuntimeTemplate template, List<EventHandlerBuildItem> axonAnnotatedBeans, BeanContainerBuildItem beanContainer) {
        axonAnnotatedBeans.forEach(item -> template.registerEventHandler(beanContainer.getValue(), item.getAxonAnnotatedClass()));
    }

    @BuildStep
    @Record(ExecutionTime.RUNTIME_INIT)
    void registerQueryHandlers(AxonRuntimeTemplate template, List<QueryHandlerBuildItem> axonAnnotatedBeans, BeanContainerBuildItem beanContainer) {
    ...
    private <T extends AxonBuildItem> void scanForBeans(BeanArchiveIndexBuildItem beanArchiveIndex,
        ...
        for (AnnotationInstance ann : testBeans) {
            try {
                ...
                Class<?> beanClass = ClassLoader.getSystemClassLoader().loadClass(beanClassInfo.name().toString());
